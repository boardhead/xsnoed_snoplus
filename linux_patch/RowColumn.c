/**
 *
 * $Id: RowColumn.c,v 1.1 2000/02/03 15:04:53 phil Exp $
 *
...
static const char rcsid[] = "$Id: RowColumn.c,v 1.1 2000/02/03 15:04:53 phil Exp $";
     ...
     va_dcl
{
    ...
    DEBUGOUT(XdbDebug("MENU", w, "%s:MenuProcEntry(%d) - %s\n",
	__FILE__, __LINE__,
	XdbMenuEnum2String(proc)
	));
    DEBUGOUT(XdbDebug("ENTRY", w, "%s:MenuProcEntry(%d) - %s\n",
	__FILE__, __LINE__,
	XdbMenuEnum2String(proc)
	));
	...
	Widget menu = w;
		...
		if (MGR_NumChildren(menu) > 0 && XmGetTearOffControl(menu) != MGR_Children(menu)[0])
		{
	...
	XmAnyCallbackStruct *cbs;
	    ...
	    DEBUGOUT(XdbDebug("ENTRY", w, "%s:MenuProcEntry(%d) - %s %sRowColumn %s entryCallbacks\n",
		__FILE__, __LINE__,
		XdbMenuEnum2String(proc),
	...
	Widget tlm;
	    ...
	    DEBUGOUT(XdbDebug0("MENU", w, "\t%s\n",
	    	XdbEventType2String(event->type)
	    	));
		...
		if (XmIsRowColumn(menu))
		{
		    ...
		    {
			DEBUGOUT(XdbDebug0("MENU", w, "\t%s %s\n",
			    XdbRcType2String(RC_Type(tlm)),
			    XtName(tlm)
			    ));
			switch(RC_Type(tlm))
			{
	...
	unsigned char string_direction;
	    ...
	    DEBUGOUT(XdbDebug0(__FILE__, cb, "\t%s sub-menu %s\n",
	    	XdbRcType2String(RC_Type(XtParent(cb))),
	    	sub_pane ? XtName(sub_pane) : "None"
	    	));
	    DEBUGOUT(XdbDebug0("MENU", cb, "\t%s sub-menu %s\n",
	    	XdbRcType2String(RC_Type(XtParent(cb))),
	    	sub_pane ? XtName(sub_pane) : "None"
	    	));
	    if (!sub_pane)
...
RadioHandler(Widget w)
{
    ...
    DEBUGOUT(XdbDebug2(__FILE__, rc, w, "RadioHandler\n"));
     ...
     */
    if (RC_RadioAlwaysOne(rc) && (XmIsGadget(w) ? TBG_Set(w) : TB_Set(w)) == False)
    {
	int cnt = 0;
	...
	if (XmIsToggleButton(tgl_set)) {
		...
		"Call XmToggleButtonSetState(tgl, True, False)"));
	    XmToggleButtonSetState(tgl_set, True, False);
	} else if (XmIsToggleButtonGadget(tgl_set)) {
	    DEBUGOUT(XdbDebug2(__FILE__, rc, tgl_set,
		"Call XmToggleButtonGadgetSetState(tgl, True, False)"));
	    XmToggleButtonGadgetSetState(tgl_set, True, False);
	}
    }
    if (tgl_unset) {
	if (XmIsToggleButton(tgl_unset)) {
	    DEBUGOUT(XdbDebug2(__FILE__, rc, tgl_unset,
		"Call XmToggleButtonSetState(tgl, False, False)"));
	    XmToggleButtonSetState(tgl_unset, False, False);
	} else if (XmIsToggleButtonGadget(tgl_unset)) {
	    DEBUGOUT(XdbDebug2(__FILE__, rc, tgl_unset,
		"Call XmToggleButtonGadgetSetState(tgl, False, False)"));
	    XmToggleButtonGadgetSetState(tgl_unset, False, False);
	}
    }
	...
	if (XmIsToggleButton(tgl_set)) {
	    DEBUGOUT(XdbDebug2(__FILE__, rc, tgl_set,
		"Call XmToggleButtonSetState(tgl, True, True)"));
	    XmToggleButtonSetState(tgl_set, True, True);
	} else if (XmIsToggleButtonGadget(tgl_set)) {
	    DEBUGOUT(XdbDebug2(__FILE__, rc, tgl_set,
		"Call XmToggleButtonGadgetSetState(tgl, True, True)"));
	    XmToggleButtonGadgetSetState(tgl_set, True, True);
	}
    }
#endif
}
...
Boolean validButton;
    ...
    DEBUGOUT(XdbDebug("MENU", w, "_XmMenuBtnDown - %s %s posted\n",
...
(((XmRowColumnClassRec *)(XtClass(w)))->row_column_class.menuProcedures)(XmMENU_BUTTON, w, event, &validButton);
    ...
    {
	DEBUGOUT(XdbDebug0("MENU", w, "_XmMenuBtnDown - %s\n",
	    "\tbutton not valid\n"
	    ));
    }
...
_XmMenuFocus(Widget w, int operation, Time _time)
{
    ...
    DEBUGOUT(XdbDebug("FOCUS", w, "%s(%d):_XmMenuFocus() - %s\n",
    	__FILE__, __LINE__,
    	XdbMenuFocusOp2String(operation)));
	...
	if (! SavedState)
	{
	    ...
	    state->RC_menuFocus.oldWidget = XtWindowToWidget(XtDisplay(w),
						     state->RC_menuFocus.oldFocus);
	    SavedState = True;
	    DEBUGOUT(XdbDebug("FOCUS", w, "\tsaving %s\n",
	...
	if (SavedState)
	{
	    ...
	    {
	    	// PH 02/02/00 - handle case where there is no oldWidget (oldFocus == PointerRoot)
	    	// - necessary for option menus
	    	if (state->RC_menuFocus.oldFocus) {
		...
		else
		{
		    ...
		    DEBUGOUT(XdbDebug("FOCUS", w, "  Restore focus to %s\n",
			XtName(state->RC_menuFocus.oldWidget)));
